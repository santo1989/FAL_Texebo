        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Finish Packing List</h3>
                    <div class="card-tools">
                        <a href="{{ route('finish_packing_data.create') }}" class="btn btn-sm btn-primary">Add New</a>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{ route('finish_packing_data.index') }}" method="GET" class="mb-4">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="search">Search by Style/Color</label>
                                    <input type="text" name="search" id="search" class="form-control" placeholder="Search by Style or Color" value="{{ request('search') }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                            ...
                            <div class="col-md-4 d-flex align-items-end">
                                ...
                                <a href="{{ route('finish_packing_data.index') }}" class="btn btn-secondary">Reset Filters</a>
                            </div>
                        </div>
                    </form>
                            ...
                            <tbody>
                                @forelse ($finishPackingData as $key => $data)
                                    <tr>
                                        ...
                                        @foreach ($allSizes as $size)
                                            <td>{{ $data->packing_quantities[strtolower($size->name)] ?? 0 }}</td>
                                        @endforeach
                                        <td>{{ $data->total_packing_quantity }}</td>
                                        <td>
                                            <a href="{{ route('finish_packing_data.show', $data->id) }}" class="btn btn-sm btn-info">Show</a>
                                            <a href="{{ route('finish_packing_data.edit', $data->id) }}" class="btn btn-sm btn-warning">Edit</a>
                                            <form action="{{ route('finish_packing_data.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ 6 + count($allSizes) }}" class="text-center">No finish packing data found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    {{-- Assuming you might have a report for finish packing data --}}
