                        ...
                        <form action="{{ route('finish_packing_data.update', $finishPackingDatum->id) }}" method="POST" id="editFinishPackingForm">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="{{ $finishPackingDatum->po_number }}" readonly>
                                </div>
                                    ...
                                    <div class="row">
                                        @foreach($sizeData as $size)
                                            <div class="col-md-3 mb-3">
                                                ...
                                                <small class="form-text text-muted">Current: {{ $size['packing_quantity'] }}, Max: {{ $size['max_allowed'] }}</small>
                                                <div class="text-danger" id="error_packing_quantity_{{ $size['id'] }}"></div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('input[type="number"]').forEach(input => {
                const updateValidation = () => {
                    if (input.name.includes('packing_quantities')) {
                        const max = parseInt(input.getAttribute('data-max'));
                        const value = parseInt(input.value) || 0;
                        const errorDiv = document.getElementById(`error_packing_quantity_${input.id.split('_')[2]}`);
