...
<x-backend.layouts.master>
                    ...
                    @if(old('product_combination_id') && old('quantities'))
                        ...
                        @foreach($oldSizeIds as $sizeId)
                            ...
                            @if($size)
                                <div class="col-md-2 mb-3">
                                    <div class="form-group">
                                        <label for="quantity_{{ $sizeId }}">{{ $size->name }} <small id="limit_{{ $sizeId }}">(No limit)</small></label>
                                        <input type="number" name="quantities[{{ $sizeId }}]" 
                                               id="quantity_{{ $sizeId }}" 
                                               class="form-control quantity-input"
                                               placeholder="Enter quantity"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                ...
                fetch(`/get-combination/${styleId}/${colorId}`)
                    ...
                    .then(data => {
                        if (data.success) {
                            combinationDisplay.textContent = data.combination.buyer_name;
                            combinationInput.value = data.combination.id;
                            fetchOrderAndCuttingQuantities(data.combination.id, data.combination.size_ids);
                        } else {
                            combinationDisplay.textContent = 'No combination found';
                            combinationInput.value = '';
                            sizeQuantitiesCard.style.display = 'none';
            ...
            // Function to fetch order and cutting quantities
            function fetchOrderAndCuttingQuantities(combinationId, selectedSizeIds) {
                fetch(`/get-order-and-cutting-quantities/${combinationId}`)
                    .then(response => {
                        if (!response.ok) throw new Error('Network response was not ok');
                        return response.json();
                    })
                    .then(data => {
                        const orderQuantities = data.order_quantities || {};
                        const cuttingQuantities = data.cutting_quantities || {};
                        loadSizeInputs(selectedSizeIds, orderQuantities, cuttingQuantities);
                    })
                    .catch(error => {
            ...
            // Function to load size inputs with limits
            function loadSizeInputs(selectedSizeIds, orderQuantities, cuttingQuantities) {
                sizeQuantityInputsContainer.innerHTML = '';
                    ...
                    selectedSizeIds.forEach(sizeId => {
                        fetchSizeName(sizeId).then(sizeName => {
                            if (sizeName) {
                                ...
                                const label = document.createElement('label');
                                label.setAttribute('for', `quantity_${sizeId}`);
                                const normalizedSize = sizeName.toLowerCase();
                                const orderQty = orderQuantities[normalizedSize] || 0;
                                const cuttingQty = cuttingQuantities[normalizedSize] || 0;
                                const maxLimit = orderQty ? Math.ceil(orderQty * 1.05) : 0;
                                const availableLimit = maxLimit ? Math.max(0, maxLimit - cuttingQty) : 0;
                                label.innerHTML = `${sizeName.toUpperCase()} <small id="limit_${sizeId}">${availableLimit > 0 ? '(Limit: ' + availableLimit + ')' : orderQty ? '(No available limit)' : '(No limit)'}</small>`;
                                ...
                                input.setAttribute('min', '0');
                                input.setAttribute('value', '0');
                                input.setAttribute('data-max', availableLimit);

                                // Client-side validation for max limit
                                input.addEventListener('input', function() {
                                    const max = parseInt(this.dataset.max) || 0;
                                    if (max > 0 && parseInt(this.value) > max) {
                                        this.value = max;
                                        alert(`Quantity for ${sizeName.toUpperCase()} cannot exceed ${max}`);
                                    }
                                    calculateTotalQuantity();
                                });
