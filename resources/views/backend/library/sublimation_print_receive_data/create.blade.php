                ...
                if (isReceive || isWaste) {
                    ...
                    if (value > max) {
                        ...
                        Swal.fire({
                            icon: 'warning',
                            title: 'Invalid Input',
                            text: `${isReceive ? 'Receive' : 'Waste'} quantity cannot exceed available quantity (${max}).`,
                        });
                    }

                    // Extract sizeId from name
                    const name = target.name;
                    const match = name.match(/\[(sublimation_print_receive_(?:quantities|waste_quantities))\]\[(\d+)\]/);
                    const sizeId = match ? match[2] : null;

                    if (sizeId) {
                        // Find paired inputs within the row
                        const receiveInput = row.querySelector(`input[name$="\\[sublimation_print_receive_quantities\\][${sizeId}\\]"]`);
                        const wasteInput = row.querySelector(`input[name$="\\[sublimation_print_receive_waste_quantities\\][${sizeId}\\]"]`);

                        let receiveVal = parseInt(receiveInput.value) || 0;
                        let wasteVal = parseInt(wasteInput.value) || 0;
                        const available = parseInt(target.getAttribute('data-available')) || 0;
